function [PHI, S, X, Y, soln_time, A_mem] = poisson_2d_Dirichlet(dir_bcs,src_fun_name, L, H, num_pts)
Nx = num_pts;
Ny = num_pts; 
Ntot = Nx*Ny;
...
% compute value of f(x,y) at Dirichlet points
D = feval(dir_bcs,x,y,L,H,num_pts);
% allocate memory for the matrix and RHS vector
A = spalloc(Ntot,Ntot,5*Ntot); %matrix A memory allocation
b = S(:); %The solution matrix A
...
for j=1:Ny
  n = ((i-1)*Ny) + j;
  A(n,n) = 1;
  b(n) = D(i,j); % B equals to the respective dirichlet value at the boundary
end
% BC2: x=L, y=y
% fill in
i = Nx;
